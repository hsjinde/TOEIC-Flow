...
export const ReadingPassageView: React.FC<ReadingPassageViewProps> = ({ passage, onComplete }) => {
  ...
  const PassageBox = (
    <div className="p-5 rounded-2xl bg-card border border-muted text-sm leading-relaxed space-y-4 max-h-[60vh] md:max-h-[70vh] overflow-y-auto shadow-sm">
  ...
  const QuestionBox = currentQ && currentBlank ? (
    <div className="space-y-4">
      <div className="p-4 rounded-2xl bg-card border border-muted shadow-sm">
        <div className="text-xs text-primary font-semibold mb-1">
          題目 {currentQ.number} ({currentQIndex + 1}/{passage.questions.length})
        </div>
  ...
  return (
    <div className="w-full">
      {/* Mobile Tab Selector (Hidden on desktop md:) */}
      <div className="grid grid-cols-2 p-1 rounded-xl bg-muted text-xs font-semibold mb-4 md:hidden">
        <button
      ...
      <div className="block md:hidden">
        {activeTab === 'passage' ? (
          <div className="space-y-4">
            {PassageBox}
            <Button variant="secondary" onClick={() => setActiveTab('questions')} className="text-xs">
              前往回答題目 ➔
            </Button>
          </div>
        ) : (
      ...
      {/* Desktop View: Side-by-Side 2 Columns Layout */}
